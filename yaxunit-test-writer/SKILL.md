...
- **Компонентный** (метод обращается к библиотечным методам, методам других безнес-правил) - если замокирован, то в транзакции, если мокирование невозможно и вызываемый метод обращается к БД - в транзакции
...
- Вспомогательные методы (хелперы) размещаются в области `СлужебныеПроцедурыИФункции` — они **не экспортные**. Типичные хелперы: создание тестовых объектов (`СоздатьГруппу`, `СоздатьТовар`), построение коллекций (`КолеровочнаяКарта`, `Товары`), добавление строк (`ДобавитьВКарту`)
